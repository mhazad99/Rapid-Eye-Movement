...
class SubstractSignals(SciNode):
    """"
    Substract a signal to others.
    ...
    -----------
        ...
        channel_to_sub: String
            A string with the name of the channel to substract
        new_channel_name : String
            To rename the new substracted channel.
            Valid only when a single channel is substracted.
            The default name is "channel - channel to substract".
    ...
    def __init__(self, **kwargs):
        ...
        if DEBUG: print('SubstractSignals.__init__')
        self._filename = None
    ...
    def compute(self, signals, channel, channel_to_sub, new_channel_name):
        ...
        if DEBUG: print('SubstractSignals.compute')
        # Clear the cache (usefull for the second run)
        self.clear_cache() # It  makes the cache=None        

        if isinstance(signals,str) and signals=='':
            raise NodeInputException(self.identifier, "signals", f"SubstractSignals this input is not connected.")
        elif not isinstance(signals,list):
            raise NodeInputException(self.identifier, "signals", \
                f"SubstractSignals input of wrong type. Expected: <class 'list'> received: {type(signals)}")     
        elif len(signals)< 2:
            raise NodeInputException(self.identifier, "signals", \
                f"SubstractSignals at least 2 signals is expected for the operation")      
        ...
        elif ((len(channel)>0) and (channel in channel_lst)) and (len(channel_to_sub)>0) and (channel_to_sub in channel_lst):
            signal_chan_1 = SignalModel.get_attribute(signals, None, 'channel', value_to_test=channel)
            signal_chan_2 = SignalModel.get_attribute(signals, None, 'channel', value_to_test=channel_to_sub)

        else:
            err_message = " ERROR: channel or channel_to_sub not in signals"

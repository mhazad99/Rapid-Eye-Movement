...
"""
"""
    Results viewer of the DerivativeApprox plugin
"""
...
from RapidEyeMovementModules.DerivativeApprox.Ui_DerivativeApproxResultsView import Ui_DerivativeApproxResultsView
from matplotlib.backends.backend_qt5agg import FigureCanvasQTAgg as FigureCanvas
from matplotlib.backends.backend_qt5agg import NavigationToolbar2QT as NavigationToolbar
from matplotlib.figure import Figure
import numpy as np

class DerivativeApproxResultsView(Ui_DerivativeApproxResultsView, QtWidgets.QWidget):
    """
        DerivativeApproxResultsView nohting to show.
    """
    def __init__(self, parent_node, cache_manager, pub_sub_manager, *args, **kwargs):
        super(DerivativeApproxResultsView, self).__init__(*args, **kwargs)
    ...
    def _plot_det_info( self, signals):
        ...
        for signal in signals:
            ...
            # plot derivative
            if n_chan>1:
                ax1[chan_sel].plot(time_vect, signal.meta['derivative'], 'r', linewidth=1, alpha=0.75)
            else:
                ax1.plot(time_vect, signal.meta['derivative'], 'r', linewidth=1, alpha=0.75)

            # Add vertical lines for sec
            nsec = int(duration)
            for sec_i in range(nsec):
                if n_chan>1:
                    ax1[chan_sel].vlines(x=sec_i, ymin=min(signal.meta['derivative']),\
                        ymax=max(signal.meta['derivative']), linewidth=0.5, color='b', linestyles='--') 
                else:
                    ax1.vlines(x=sec_i, ymin=min(signal.meta['derivative']),\
                         ymax=max(signal.meta['derivative']), linewidth=0.5, color='b', linestyles='--')                     

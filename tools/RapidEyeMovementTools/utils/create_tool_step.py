...
while not done:
    ...
    if is_custom_decision.lower() == "y" or is_custom_decision == "":
        ...
        tool_description_path = os.path.join(tool_path, "description.json")
        with open (tool_description_path, "r") as f:
            tool_description = json.loads(f.read())
            tool_description["steps_description"].append(
                {
                    "name": step_label,
                    "description": step_description,
                    "custom_step_name":step_class,
                    "show_index": True if len(tool_description["steps_description"]) > 0 else False
                }
            )
